...
class otpAgricoinsController extends Controller
{
    ...
    function otpAgriCoinsConfirm(Request $request){
        ...
        if($verify_otp != NULL){
            ...
            Order::where('order_number', $request->order_num)->delete();
            SubOrder::where('order_id', $order_obj->id)->delete();
    ...
    function confirmViaButton($otp_token, $order_num){
        ...
        if($verify->status == 'true'){
            ...
            $order_obj_tmp = Order::where('order_number', $order_num)->first();
            // $sub_order_obj = SubOrder::where('order_id', $order_obj_tmp->id)->delete();
            // $order_obj = Order::where('order_number', $order_num)->delete();
            DB::table('orders')->where('order_number', $order_num)->delete();
            DB::table('sub_orders')->where('order_id', $order_obj_tmp->id)->delete();
              // notification entity
              $notification_ent = new notification();
              $notification_ent->user_id =  Auth::user()->id;
              $notification_ent->frm_user_id = '1';
           ...
           // view otp routes coins
           return view('otp_ag_coins.order_conf_agcoins_otp_status')->with('otp_verification', $otp_verification)->with('order_obj', $order_obj_tmp);
    }
