            ...
            <div class="row" onload="defValue()">
                ...
                <div class="col-md-12 col-lg-5 col-12">
                    <div class="product-details-content">
                        ...
                        @if($productAveRating != '0' || $productAveRating != NULL)
                            <div class="rating-number">
                                <div class="quick-view-rating">
                                    {{$productAveRating}}
                                    <a href="">
                                        <span class="fa fa-star {{(floatval($productAveRating) >= 1) ? 'checked' : '' }}"></span>
                                    </a>
                                    <a href="">
                                        <span class="fa fa-star {{(floatval($productAveRating) >= 2) ? 'checked' : '' }}"></span>
                                    </a>
                                    <a href="">
                                        <span class="fa fa-star {{(floatval($productAveRating) >= 3) ? 'checked' : '' }}"></span>
                                    </a>
                                    <a href="">
                                        <span class="fa fa-star {{(floatval($productAveRating) >= 4) ? 'checked' : '' }}"></span>
                                    </a>
                                    <a href="">
                                        <span class="fa fa-star {{(floatval($productAveRating) >= 5) ? 'checked' : '' }}"></span>
                                    </a>
                                </div>
                            </div>
                        @else
                        ...
                        <div class="fb-share-button mb-3" data-layout="button" data-size="large"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u={{url()->current()}}" class="fb-xfbml-parse-ignore">Share</a></div>

                        <!-- changelog change GET to POST -->
                        <form method="GET" action="{{route('cart.addWquantityVariation', $product)}}" id="add--cart-form">
                            <input type="hidden" id="isWholeSale" name="isWholeSale" value="{{ $product->is_whole_sale }}">
                            <input type="hidden" id="wholeSaleMinQty" name="wholeSaleMinQty" value="{{ $product->whole_sale_min_qty }}">
                            @php
                            ...
                            @endphp
                                ...
                                <div class="quick-view-select" {{ $hidden_product }}>
                                    <div class="select-option-part">
                                        ...
                                        @php
                                            $var_id = 0;
                                            if ( count( $product_variations ) > 0 && $product_variations[0]->variation_name == "Regular" ) {
                                                $var_id = $product_variations[0]->id;
                                            }
                                        @endphp
                                        <input type="hidden" required id="variation_id_setter" name="variation_id" value="{{ $var_id }}">                            
                                        <!-- {{ $product_variation_prices }} -->
                                    </div>
                                ...
                                @if ( $product->is_whole_sale )
                                    <div class="form-group row">
                                        <div class="col-12">
                                            Note: Minimum purchase quantity is {{ number_format( $product->whole_sale_min_qty ) }}
                                        </div>
                                    </div>
                                @endif
                                <div class="form-group row">
                                    <div class="col-4">
                                        <input class="input-form" name="quantity" require type="number" value="1" id="variation_max_stock" min="{{ $product->is_whole_sale ? $product->whole_sale_min_qty : 1 }}">
                                    </div>
                                    <div class="col-8">
    ...
    <script>
        window.onload = function() {
            ...
            /* $( document ).on( 'submit', '#add--cart-form', function( e ) {
                const isWholeSale = $( '#isWholeSale' ).val()
                const wholeSaleMinQty = $( '#wholeSaleMinQty' ).val()

                console.log( isWholeSale, wholeSaleMinQty )
                e.preventDefaul()
            } ) */
        }
    </script>
