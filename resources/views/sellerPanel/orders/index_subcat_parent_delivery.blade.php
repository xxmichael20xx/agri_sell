    ...
    <div class="nav-tabs-wrapper">
        <ul id="tabs" class="nav nav-tabs" role="tablist">
            ...
            <li class="nav-item active">
                <a class="nav-link" href="/sellerpanel/manage_orders/delivery/{{$status->id}}" 
                    >{{$status->display_name}}</a>
            </li>
            @elseif ($delivery_status_counter != 7)
            {{-- Not delivery status which is hidden --}}
            <li class="nav-item">
                <a class="nav-link" href="/sellerpanel/manage_orders/delivery/{{$status->id}}" >{{$status->display_name}}</a>
            </li>
            @endif
            @php
